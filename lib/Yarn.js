...
return (function(){ var yarn, csi;
  ...
  yarn.toString = function (containing) { var used, out;
    out = new(Array), used = new(Array);
    
    this.styles.forEach(function (style) { var code;
      if (typeof containing === 'undefined' ||
          !containing.some(function (cStyle) {
        if (Array.isArray(cStyle)) { return(cStyle.indexOf(style) !== -1) }
                              else { return(cStyle === style) }
        })) {
          used.push(style);
  ...
  yarn.ansiCodes = { 'reset': 0,
    ...
    'fg-black-hi':   90, 'bg-black-hi':   100,
    'fg-red':        31, 'bg-red':        41,
    'fg-red-hi':     91, 'bg-red-hi':     101,
    'fg-green':      32, 'bg-green':      42,
    'fg-green-hi':   92, 'bg-green-hi':   102,
    'fg-yellow':     33, 'bg-yellow':     43,
    'fg-yellow-hi':  93, 'bg-yellow-hi':  103,
    'fg-blue':       34, 'bg-blue':       44,
    'fg-blue-hi':    94, 'bg-blue-hi':    104,
    'fg-magenta':    35, 'bg-magenta':    45,
    'fg-magenta-hi': 95, 'bg-magenta-hi': 105,
    'fg-cyan':       36, 'bg-cyan':       46,
    'fg-cyan-hi':    96, 'bg-cyan-hi':    106,
    'fg-white':      37, 'bg-white':      47,
    'fg-white-hi':   97, 'bg-white-hi':   107
  };
