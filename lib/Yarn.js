...
return (function(){ var yarn;
  ...
  // Constructs a new `yarn`, given styles, labels, and some other `String`s
  // or `yarns`.
  yarn.constructor = function (blueprint) {
  ...
  yarn.style = function (styles) {
    this.styles = process.mixin(null, this.styles, styles); return this; };
  ...
  // labels
  yarn.styling = function (quilt) { var styles;
    styles = new(Object);
    
    this.labels.forEach(function (label) {
      if (typeof quilt[label] !== 'undefined') {
        styles = process.mixin(null, quilt[label], styles) } });
    
    return process.mixin(null, styles, this.styles);
  };
  ...
  //       info.
  yarn.toString = function (quilt, containers) { var out, codes;
           out = new(Array),
         quilt = quilt || new(Object),
    containers = containers || new(Array);
    
    if ((codes = this.applyCodes(quilt)).length > 0) {
      out.push(yarn.codes['csi'], codes.join(';'), yarn.codes['sgr']) };
    
    containers.push(this);
    this.elements.forEach(function (element) {
      out.push( element.toString(quilt, containers) ) });
    containers.pop();
  ...
  yarn.applyCodes = function (quilt) { var codes, styles;
    codes = new(Array);
  ...
  yarn.unapplyCodes = function (containers, quilt) { var codes, styles;
    codes = new(Array);
